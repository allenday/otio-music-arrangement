...
def test_calculate_subdivision_markers_irregular_beats():
    ...
    markers = timing_utils.calculate_subdivision_markers(beats, 2, start_time)
    expected_keys = {
        tk(0.0), tk(0.35), # Interval 0.7 / 2 = 0.35
        tk(0.7), tk(1.1),  # Interval 0.8 / 2 = 0.4 -> 0.7 + 0.4 = 1.1
        tk(1.5), tk(1.75), # Interval 0.5 / 2 = 0.25 -> 1.5 + 0.25 = 1.75
        tk(2.0)
    }
    # Use pytest.approx for potential floating point comparisons if needed, though RationalTime helps
    assert set(markers.keys()) == expected_keys
    assert markers[tk(0.0)] == "Beat 1"
...
def test_adjust_segments_start_before_first_db():
    ...
    adjusted = timing_utils.adjust_segment_times_to_downbeats(segments, downbeats_offset, start_time)
...
def test_adjust_segments_between_dbs():
    ...
    # Correction: Logic snaps start to 2.0, end would snap to 4.0. But if start>end, snaps both to start. Check code.
    # -> Code handles start>end case by snapping end to start's snap point.
    # So, start snaps to 2.0. End *tries* to snap to 4.0.
    # Since 2.0 < 4.0, it should keep these.
    assert adjusted[0]["adjusted_start_time"] == rt(2.0)
    assert adjusted[0]["adjusted_end_time"] == rt(4.0)

def test_adjust_segments_very_short_between_dbs():
    """Test very short segment entirely between two downbeats (causes start > end initially)."""
    # Use downbeats closer together for clarity
    short_db = [0.0, 0.5, 1.0]
    segments = [{"start": 0.6, "end": 0.9, "label": "A"}]
    adjusted = timing_utils.adjust_segment_times_to_downbeats(segments, short_db, start_time)
    assert len(adjusted) == 1
    ...
    # Start snaps to 0. End snaps to 1. OK.
    # The warning in the code `start > end` seems to handle the case where the segment itself
    # is defined such that its start > end *before* snapping, which shouldn't happen with valid input.
    # Let's re-read the code block around line 130.
    # Ah, the `nearest_start_db_rt > nearest_end_db_rt` check handles when the *chosen snap points* are inverted.
    # This happens if a segment is *entirely* contained between two downbeats, e.g., db=[0, 2], seg=[0.1, 0.2].
    ...
    # Let's test the case mentioned in the warning: Segment 0.1 to 0.9, downbeats [0.0, 1.0]
    segments_short = [{"start": 0.1, "end": 0.9, "label": "Short"}]
    dbs_simple = [0.0, 1.0]
    adjusted_short = timing_utils.adjust_segment_times_to_downbeats(segments_short, dbs_simple, start_time)
    assert adjusted_short[0]["adjusted_start_time"] == rt(0.0)
    assert adjusted_short[0]["adjusted_end_time"] == rt(1.0)
    # The warning seems to describe a scenario that the snapping logic prevents.
...
def test_adjust_segments_prevent_overlap():
    ...
    segments = [
        ...
        {"start": 2.5, "end": 5.5, "label": "B"}, # Ideal Snap: 2.0 - 6.0. Adjusted Snap: 4.0 - 6.0
    ]
    adjusted = timing_utils.adjust_segment_times_to_downbeats(segments, db, start_time)
    assert len(adjusted) == 2
    assert adjusted[0]["adjusted_start_time"] == rt(0.0)
    assert adjusted[0]["adjusted_end_time"] == rt(4.0)
    assert adjusted[1]["adjusted_start_time"] == rt(4.0) # Adjusted from 2.0 to prevent overlap
    assert adjusted[1]["adjusted_end_time"] == rt(6.0)
...
def test_adjust_segments_zero_duration_overlap():
    ...
    segments = [
        {"start": 0.1, "end": 0.2, "label": "A"}, # Snaps: 0.0 - 0.0 (due to start>end rule)
        {"start": 0.3, "end": 1.8, "label": "B"}, # Ideal Snap: 0.0 - 2.0. Adjusted: 0.0 - 2.0
    ]
    dbs_tight = [0.0, 2.0, 4.0]
    adjusted = timing_utils.adjust_segment_times_to_downbeats(segments, dbs_tight, start_time)
    assert len(adjusted) == 2
    ...
    # start(0) < end(1) -> ok. adjusted=[0.0, 1.0]
    # The warning seems wrong, or I misunderstand when `nearest_start_db_rt > nearest_end_db_rt` triggers.
...
# TODO: Add tests for different time rates if the logic depends on it (it shouldn't much) 