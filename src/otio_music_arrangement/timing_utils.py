...
def adjust_segment_times_to_downbeats(segments, downbeats, global_start_time):
    """Adjusts segment start/end times to align with nearest downbeats.

    Args:
        segments (list): List of segment dictionaries, each needing 'start', 'end', 'label'.
                         Start/end times are expected as seconds (float/int).
        downbeats (list): List of downbeat times in seconds (float/int).
        global_start_time (opentime.RationalTime): The zero time for the timeline.

    Returns:
        list: Adjusted segments with 'adjusted_start' and 'adjusted_end' keys
    ...
    if not downbeats:
        logger.warning('Adjusting segment times: No downbeats provided. Returning original times.')
        return [
            {
                'label': seg.get('label', f'Segment {i+1}'),
                'original_start_time': opentime.RationalTime(time_value(seg.get('start', 0)), global_start_time.rate),
                'original_end_time': opentime.RationalTime(time_value(seg.get('end', 0)), global_start_time.rate),
                'adjusted_start_time': opentime.RationalTime(time_value(seg.get('start', 0)), global_start_time.rate),
                'adjusted_end_time': opentime.RationalTime(time_value(seg.get('end', 0)), global_start_time.rate),
    ...
    try:
        ...
        sorted_downbeats_rt = sorted([opentime.RationalTime(time_value(d), rate) for d in downbeats])
        ...
        return [
            {
                'label': seg.get('label', f'Segment {i+1}'),
                'original_start_time': opentime.RationalTime(time_value(seg.get('start', 0)), rate),
                'original_end_time': opentime.RationalTime(time_value(seg.get('end', 0)), rate),
                'adjusted_start_time': opentime.RationalTime(time_value(seg.get('start', 0)), rate),
                'adjusted_end_time': opentime.RationalTime(time_value(seg.get('end', 0)), rate),
            } for i, seg in enumerate(segments)
        ]
    ...
    for i, seg in enumerate(segments):
        ...
        try:
            ...
            original_start_rt = opentime.RationalTime(time_value(seg.get('start', 0)), rate)
            original_end_rt = opentime.RationalTime(time_value(seg.get('end', 0)), rate)
        except Exception as e:
            logger.error('Error converting segment %d times to RationalTime: %s. Segment: %s', i, e, seg, exc_info=True)
        ...
        if possible_starts:
            nearest_start_db_rt = max(possible_starts)
             ...
             # If start is before the first actual downbeat, snap start to the first downbeat
             logger.debug(
                 'Segment \'%s\' starts at %s, before first downbeat %s. Adjusting start to first downbeat.',
                 label, original_start_rt, first_downbeat_rt
             )
             nearest_start_db_rt = first_downbeat_rt
        else: # No downbeats at all
             nearest_start_db_rt = original_start_rt # Should have been caught earlier, but safe fallback
        ...
        if possible_ends:
            ...
            if original_end_rt > last_db:
                 logger.debug(
                     'Segment \'%s\' ends at %s, after last downbeat %s. Adjusting end to last downbeat.',
                     label, original_end_rt, last_db
                 )
                 ...
                 possible_ends_preceding = [db for db in sorted_downbeats_rt if db <= original_end_rt]
                 if possible_ends_preceding:
                     nearest_end_db_rt = max(possible_ends_preceding)
                     logger.debug(
                         'Segment \'%s\' ends at %s, between downbeats. Adjusting end to preceding downbeat %s.',
                         label, original_end_rt, nearest_end_db_rt
                     )
                 elif first_downbeat_rt is not None: # Ends before even the first downbeat
                      nearest_end_db_rt = first_downbeat_rt
                      logger.debug(
                          'Segment \'%s\' ends at %s, before first downbeat. Adjusting end to first downbeat %s.',
                          label, original_end_rt, nearest_end_db_rt
                      )
                 else: # No downbeats at all
        ...
        if nearest_start_db_rt > nearest_end_db_rt:
             logger.warning(
                 'Segment \'%s\' initial snap resulted in start %s > end %s. Snapping both to start %s.',
                 label, nearest_start_db_rt, nearest_end_db_rt, nearest_start_db_rt
             )
             # If initial snap makes start > end, usually means segment is tiny and fits between dbs.
             # Snapping both to the *start* db seems most logical.
        ...
        if adjusted_start_final < last_adjusted_end_time:
             logger.warning(
                 'Adjusted start time %s for segment \'%s\' overlaps previous adjusted end %s. Adjusting start to match previous end.',
                 adjusted_start_final, label, last_adjusted_end_time
             )
        ...
        if adjusted_end_final < adjusted_start_final:
             logger.warning(
                 'Segment \'%s\' adjusted end %s is before adjusted start %s after overlap correction. Setting end equal to start.',
                 label, adjusted_end_final, adjusted_start_final
             )
             adjusted_end_final = adjusted_start_final

        logger.debug(
            'Segment \'%s\': Original (%s - %s), Adjusted (%s - %s)',
            label, original_start_rt, original_end_rt, adjusted_start_final, adjusted_end_final
        )
...
def calculate_subdivision_markers(beats, subdivision_level, global_start_time):
    """Calculates marker times and labels for beat subdivisions.

    Does NOT perform frame snapping. Returns RationalTime objects.

    Args:
        beats (list): List of beat times in seconds (float/int).
        subdivision_level (int): The number of subdivisions per beat (e.g., 4 for 1/4 notes).
        global_start_time (opentime.RationalTime): The zero time for the timeline.

    Returns:
        dict: Dictionary mapping opentime.RationalTime to marker label string.
              Returns empty dict if subdivision_level < 1.
    """
    markers = {}
    if subdivision_level < 1:
        logger.error('Subdivision level must be 1 or greater, got: %d', subdivision_level)
        return markers
    ...
    for i, beat_time_rt in enumerate(beats_rt):
        # Use math.ceil to ensure beat numbers are 1-based and handle potential floating point issues near integers
    ...
    if subdivision_level > 1:
        for i in range(len(beats_rt) - 1): # Iterate through intervals between beats
            ...
            if interval_rt <= opentime.RationalTime(0, rate):
                logger.warning('Skipping zero or negative duration interval between beat %d (%s) and beat %d (%s)',
                             i+1, start_beat_rt, i+2, end_beat_rt)
                continue

            # Calculate time step for subdivisions within this interval
            # Need to multiply first then divide to avoid potential precision loss with RationalTime division
            # step_rt = interval_rt / subdivision_level # Less precise for RationalTime
            # Perform calculation using floating point for intermediate step, then convert back
            ...
            for k in range(1, subdivision_level): # k from 1 to subdivision_level-1
                ...
                intermediate_time_rt = opentime.RationalTime(intermediate_time_val, rate)
                ...
                if intermediate_time_rt >= end_beat_rt:
                    # logger.debug('Skipping subdivision %d/%d for beat %d, calculated time %s >= next beat %s',
                    #              k, subdivision_level, beat_num, intermediate_time_rt, end_beat_rt)
                    continue

                label = f'Beat {beat_num}.{k}/{subdivision_level}' # Label like Beat 5.1/4, Beat 5.2/4 etc.
                # Use a hashable key (tuple) instead of RationalTime directly
                markers[(intermediate_time_rt.value, intermediate_time_rt.rate)] = label

    logger.debug('Calculated %d markers for subdivision level %d', len(markers), subdivision_level)
